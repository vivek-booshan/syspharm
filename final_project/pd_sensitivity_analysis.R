...
for (i in 1:2) {
    ...
    p = ggplot(data=sens_mat, aes(x=weights, y=clearances, fill=sens), cex.lab=2) + 
        ...
        scale_fill_distiller(name='Expected Weight Loss', palette='RdBu') +
        scale_x_discrete(breaks=seq(min(get(volume[i])), max(get(volume[i])), 0.15))
    print(p)
    ggsave(paste0("pd_sensitivity_", tag[i], ".png"), bg="white", dpi=600, height=6.5, width=8)
}
...
ggsave(paste0("pd_sensitivity_c.png"), bg="white", dpi=600, height=6.5, width=8)
print(p)
