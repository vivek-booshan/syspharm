...
for (i in 2:5) {
    p = ggplot() + 
        ...
        labs(y=paste0(headers[i], " (", units[i], ")"), x = "weight (kgs)") +
        labs(title=paste("Overlay of Allometric", headers[i], "with Real", headers[i])) +
        theme_bw()
...
p = ggplot(data=longTapData) + 
    ...
    scale_y_log10() +
    labs(title="Distribution of Parameter Values") +
    labs(x="Measured and Predicted Parameters", y="Arbitrary Log10 Units") +
    theme_bw()
...
for (i in 2:5) {
    p = ggplot() + 
        ...
        labs(title=paste(
                ...
                "with Allometric", headers[i]), 
            x=paste0("Real ", headers[i], "(", units[i], ")"), 
            y=paste0("Allometric ", headers[i], "(", units[i], ")")) +
        theme_bw()
